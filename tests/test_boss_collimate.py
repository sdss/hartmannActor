...
config = {
    "spec": {"specs": "sp1 sp2", "cameras": "r1 b1 r2 b2"},
    "m": {"b1": 1, "r1": 1, "b2": 1, "r2": 1},
    "b": {"b1": -0.44, "r1": -0.20, "b2": -0.10, "r2": 0.20},
    "constants": {"bsteps": 319.0, "badres": 4.0, "focustol": 0.20},
    "coeff": {"r1": 439.8, "b1": 428.5, "r2": 420.8, "b2": 420.3},
}
...
notFocused_moves = {"sp1": -141.0, "sp2": 3311.5}
# ########################################
...
b1 = boss_collimate.OneCamResult(
    "b1", True, xshift, b1coeff, 30, -0.5, -2157, False, None
)
r1 = boss_collimate.OneCamResult(
    "r1", True, xshift, r1coeff, 34, -0.3, -1921, True, None
)
b2 = boss_collimate.OneCamResult("b2", True, xshift, b2coeff, 48, 0.4, 903, True, None)
r2 = boss_collimate.OneCamResult(
    "r2", True, xshift, r2coeff, 50, 0.5, 3638, False, None
)
...
class TestOneCam(hartmannTester.HartmannTester, unittest.TestCase):
    def setUp(self):
        ...
        self.oneCam = boss_collimate.OneCam(
            None, None, constants["bsteps"], constants["focustol"], coeff, 0, 0, ""
        )
        self.spec = "sp2"
        self.cam = "r2"
    ...
    def test_load_data_NeOff(self):
        ...
        self._load_data_not_ok(
            expnum1, expnum2, 1, "Incorrect header values in fits file."
        )
    ...
    def test_load_data_BothOff(self):
        ...
        self._load_data_not_ok(
            expnum1, expnum2, 2, "Incorrect header values in fits file."
        )
    ...
    def test_call_bad_cam(self):
        ...
        self.assertEqual(
            self.oneCam.messages[0][1], 'text="I do not recognize camera notACam"'
        )
...
class TestHartmann(hartmannTester.HartmannCallsTester, unittest.TestCase):
    ...
    def test_collimate_focused(self):
        ...
        for spec in self.hart.full_result:
            for cam in self.hart.full_result[spec]:
                self.assertEqual(
                    self.hart.full_result[spec][cam].focused, focused_focused[spec][cam]
                )
    ...
    def test_collimate_notFocused(self):
        ...
        for spec in self.hart.full_result:
            for cam in self.hart.full_result[spec]:
                self.assertEqual(
                    self.hart.full_result[spec][cam].focused,
    ...
    def test_collimate_bad_ffs_bypassed(self):
        ...
        self.hart.collimate(
            exp1, exp2, indir=badFFS_dir, cmd=self.cmd, plot=False, bypass=["ffs"]
        )
