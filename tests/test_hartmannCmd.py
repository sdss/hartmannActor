...
class TestHartmannCmd(HartmannCmdTester, unittest.TestCase):
    ...
    def test_recompute_fails(self):
        ...
        self._recompute(
            "id={expnum1} id2={expnum2} mjd={mjd}".format(**expect),
            expect,
            success=False,
        )
    ...
    def test_recompute_bypass(self):
        ...
        self._recompute(
            'id={expnum1} mjd={mjd} bypass="ffs"'.format(**expect), expect, success=True
        )
    ...
    def _collimate(self, args, expect, success=True):
        ...
        self.assertEqual(
            hart.kwargs["ignoreResiduals"], expect.get("ignoreResiduals", False)
        )
        self.assertEqual(hart.kwargs["noCheckImage"], expect.get("noCheckImage", False))
