...
class TestHartmannCmd(HartmannCmdTester,unittest.TestCase):
    ...
    def test_recompute_no_move_no_subFrame(self):
        currentMJD = int(astroMJD.mjdFromPyTuple(time.gmtime())+0.3)
        expect = {'expnum1':3, 'mjd':currentMJD, 'moveMotors':False, 'subFrame':False}
        self._recompute('id={expnum1} noCorrect noSubframe'.format(**expect), expect, success=True)
    ...
    def test_collimate_ok(self):
        expect = {'expnum1':1}
        self._collimate('id={expnum1}'.format(**expect), expect, success=True)
    def test_collimate_no_move_no_subFrame(self):
        expect = {'expnum1':3, 'moveMotors':False, 'subFrame':False}
        self._collimate('id={expnum1} noCorrect noSubframe'.format(**expect), expect, success=True)
    def test_collimate_fails(self):
        expect = {'expnum1':4,}
        self._collimate('id={expnum1}'.format(**expect), expect, success=False)
